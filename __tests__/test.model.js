...
describe("Model Structure", () => {
  test("should have all expected categories", () => {
    expect(model.categories).toEqual(['baguette', 'canapes', 'coffee', 'themeCake']);
  });
  ...
  describe("Products", () => {
    ...
    test("should have correct product details", () => {
      expect(model.products).toContainEqual({ id: 1, name: 'Baguette', price: 5 });
      expect(model.products).toContainEqual({ id: 2, name: 'Coffee', price: 3 });
      expect(model.products).toContainEqual({ id: 3, name: 'Canape', price: 4 });
    });
  });
});