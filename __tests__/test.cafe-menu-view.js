...
jest.mock('../src/cafe-menu-view', () => ({
  ...
  createProductCardHtml: jest.fn((product) => `<div class="product-card">${product.productName} ${product.unitPrice} Kr</div>`)
}));

import { createCafeMenuHtml, createThemeCakeMenuHtml, createProductCardHtml } from '../src/cafe-menu-view';
...
describe("Cafe Menu View", () => {
  beforeEach(() => {
    ...
    global.model = {
      ...
      products: [
        { productId: 1, productName: "Baguette", image: "baguette.jpg", unitPrice: 50, categoryIndex: 0 },
        { productId: 2, productName: "Coffee", image: "coffee.jpg", unitPrice: 30, categoryIndex: 2 }
      ]
    };
  });
  ...
  describe("createProductCardHtml", () => {
    test("should create product card HTML with all details", () => {
      const product = { productId: 1, productName: "Test Product", image: "test.jpg", unitPrice: 100 };
