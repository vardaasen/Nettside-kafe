...
jest.mock('../src/cafe-menu-view', () => ({
  createCafeMenuHtml: jest.fn(() => '<div id="cafe-menu-tab-row"><button>Baguette</button></div>'),
  createThemeCakeMenuHtml: jest.fn(() => '<div id="cards-grid-container"><div class="product-card"></div></div>')
}));

import { updateView } from '../src/view';
import { createCafeMenuHtml, createThemeCakeMenuHtml } from '../src/cafe-menu-view';
...
describe("View Updates", () => {
    ...
    updateView.mockImplementation(() => {
      ...
      if (appElement) {
        appElement.innerHTML = 
          model.app.currentPage === 'cafeMenu' ? createCafeMenuHtml() :
          model.app.currentPage === 'themeCake' ? createThemeCakeMenuHtml() :
          '';
      }
    });
  });
  ...
  test("should update view with cafe menu", () => {
    updateView();
    expect(document.getElementById('app').innerHTML).toContain('cafe-menu-tab-row');
    expect(document.getElementById('app').innerHTML).toContain('<button>Baguette</button>');
  ...
  test("should update view with theme cake menu", () => {
    ...
    updateView();
    expect(document.getElementById('app').innerHTML).toContain('cards-grid-container');
    expect(document.getElementById('app').innerHTML).toContain('<div class="product-card"></div>');
