const productEditController = {
  init: function() {
    ...
    document.querySelector("h1").innerHTML = `Rediger <em>${product.productName}</em>`;
  ...
  saveProduct: function(event) {
    ...
    const updatedProduct = {
      ...
      customizationAvailable: document.getElementById("customizationAvailable")?.checked,
    };

    const productIndex = model.products.findIndex(p => p.productId === this.product.productId);
    if (productIndex !== -1) {
  ...
  saveTheme: function(event, themeId) {
    ...
    let themeIndex = model.themes.findIndex(theme => theme.themeId === themeId);

    if (themeIndex === -1) {
      console.log("Adding new theme to model");

      const nextId = model.themes.length > 0 ? Math.max(...model.themes.map(t => t.themeId)) + 1 : 1;
      themeId = nextId;
      ...
      model.themeImages[themeId] = document.getElementById("themeImageUrl").value;
    } else {
  ...
  addTheme: function(event) {
    ...
    const newThemeId = model.themes.length > 0 ? Math.max(...model.themes.map(t => t.themeId)) + 1 : 1;
  ...
  deleteTheme: function(event, themeId) {
    ...
    if (model.themes.length > 0) {
      ...
      document.getElementById("themeDetailsContainer").innerHTML = "<p>Ingen temaer tilgjengelig.</p>";
    }
...
document.addEventListener("DOMContentLoaded", () => productEditController.init());
